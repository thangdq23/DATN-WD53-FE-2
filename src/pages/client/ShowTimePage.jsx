...
const ShowtimePage = () => {
  ...
  const otherMovies = (listRes?.data || []).filter((m) => m._id !== id).slice(0, 4);
  ...
  if (loadingDetail) return <div className="h-screen flex items-center justify-center bg-slate-900 text-white">Đang tải...</div>;
  if (!movie) return <div className="h-screen flex items-center justify-center bg-slate-900 text-white">Không tìm thấy phim</div>;
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="relative h-[480px] md:h-[550px] bg-slate-900 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 h-full flex items-center gap-10">
           ...
           <div className="hidden md:block w-[300px] shrink-0 rounded-lg overflow-hidden shadow-2xl shadow-black/60 border border-white/10">
              <img src={movie.poster} alt={movie.name} className="w-full h-auto object-cover" />
           </div>
           ...
           <div className="flex-1 text-white">
              <div className="flex items-center gap-3 mb-4">
                 <span className={`px-3 py-1 rounded text-xs font-bold uppercase tracking-wide ${
                    movie.statusRelease === 'nowShowing' ? 'bg-red-600' : 'bg-blue-600'
                 }`}>
                    {movie.statusRelease === 'nowShowing' ? 'Đang chiếu' : 'Sắp chiếu'}
                 </span>
                 {movie.ageRequire && (
              ...
              <div className="text-gray-300 space-y-2 mb-8 text-sm md:text-base">
                 <p><span className="text-white/60 font-medium">Thể loại:</span> {movie.genreIds?.map(g => g.name).join(", ")}</p>
                 <p><span className="text-white/60 font-medium">Thời lượng:</span> {movie.duration} phút</p>
                 <p><span className="text-white/60 font-medium">Khởi chiếu:</span> {dayjs(movie.releaseDate).format("DD/MM/YYYY")}</p>
                 <p><span className="text-white/60 font-medium">Đạo diễn:</span> {movie.director}</p>
                 <p><span className="text-white/60 font-medium">Diễn viên:</span> {Array.isArray(movie.actor) ? movie.actor.join(", ") : movie.actor}</p>
              </div>

              <div className="flex gap-4">
                 <button 
                   onClick={() => document.getElementById('showtime-section')?.scrollIntoView({ behavior: 'smooth' })}
                 ...
                 <button 
                   onClick={() => setTrailerModal({ open: true, url: normalizeYouTubeEmbed(movie.trailer) })}
            ...
            <div className="lg:col-span-4 space-y-8">
               <div>
                  ...
                  <div className="space-y-4">
                     {otherMovies.map(m => (
                       <Link key={m._id} to={`/showtime/${m._id}`} className="flex gap-4 group">
                          <img src={m.poster} alt={m.name} className="w-20 h-28 object-cover rounded bg-gray-200" />
                          <div className="flex-1">
                             <h4 className="font-semibold text-slate-800 group-hover:text-red-600 transition line-clamp-2">
                                {m.name}
                             </h4>
                             <p className="text-xs text-slate-500 mt-1">{m.genreIds?.map(g => g.name).join(", ")}</p>
      ...
      >
         <div className="aspect-video bg-black rounded-lg overflow-hidden shadow-2xl">
            {trailerModal.url && (
              <iframe width="100%" height="100%" src={trailerModal.url} title="Trailer" frameBorder="0" allowFullScreen />
            )}
         </div>
      </Modal>
    </div>
