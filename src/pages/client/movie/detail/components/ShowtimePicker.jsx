    ...
    <section id="seat-picker-container" className="mt-8">
      {!roomId || !showtimeId ? (
        <div className="max-w-5xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row">
              ...
              <div className="flex-1 flex items-center border-b md:border-b-0 md:border-r border-slate-200 relative">
                 ...
                 <button 
                   ...
                   onClick={() => {
                     const idx = days.findIndex(d => d.isSame(selectedDate, 'day'));
                   ...
                   {days.map((d) => {
                     ...
                     return (
                       <div
                         ...
                         className={`flex-shrink-0 w-24 text-center py-3 px-1 cursor-pointer transition-colors border-r border-slate-100 last:border-0 ${
                           isSelected 
                             ? "bg-[#003366] text-white" 
                             : "bg-white text-slate-600 hover:bg-slate-50"
                         }`}
                       >
                         <div className={`text-xs font-medium mb-1 ${isSelected ? "text-white/80" : "text-slate-500"}`}>
                           {dayNameCap}
                         </div>
                 ...
                 <button 
                   ...
                   onClick={() => {
                     const idx = days.findIndex(d => d.isSame(selectedDate, 'day'));
                     if(idx < days.length - 1) setSelectedDate(days[idx + 1]);
                   }}
                   disabled={days[days.length - 1].isSame(selectedDate, 'day')}
                 >
                   <RightOutlined />
                 </button>
                 </>
          ...
          {/* Showtime List */}
          {isLoading ? (
             <div className="py-12 text-center text-slate-500">Đang tải lịch chiếu...</div>
            ...
            <div className="space-y-6">
              {showtimesByRoom.map((group) => (
                <div key={group.room?._id} className="bg-white rounded-none border-b border-slate-200 pb-6 last:border-0">
