...
import { Button, Dropdown, Form, Input, InputNumber, Menu, Popconfirm, Select, Space, message, Tag } from "antd";
...
const CreateRoom = () => {
  ...
  useEffect(() => {
    const { seats } = generatePreviewSeats(rows, cols);
    setSeatState(seats);
  }, [rows, cols]);
  ...
  const handleSeatClick = (seat, event) => {
    ...
    if (isCombining) {
      ...
      if (updatedSelected.length === 2) {
        setSeatState(prevSeats => combineSeats(prevSeats, ...updatedSelected));
        setSelectedSeats([]);
  ...
  const handleUpdateSeatType = (type) => {
    setSeatState(prevSeats => {
      const selectedSeats = prevSeats.filter(seat => seat.selected && !seat.locked);
    ...
    message.success(`Đã cập nhật loại ghế cho ${selectedSeats.filter(seat => seat.selected && !seat.locked).length} ghế`);
  };

  
  const handleToggleSeatStatus = (seatId) => {
    setSeatState(prevSeats => toggleSeatStatus(prevSeats, seatId));
  };

  const toggleCombineMode = () => {
    setIsCombining(!isCombining);
    setSelectedSeats([]);
    message.info(isCombining ? 'Đã thoát chế độ ghép ghế' : 'Chọn 2 ghế liền kề để ghép');
  };
  ...
  const handleSubmit = (values) => {
    try {
      const seatsToSubmit = seatsState.map(seat => ({
        id: seat.id,
        row: seat.row,
        col: seat.col,
        label: seat.label,
        ...
        status: seat.status,
        span: seat.span || 1,
        combinedWith: seat.combinedWith || null,
        combinedSeatId: seat.combinedSeatId || null
      }));
          ...
          <div className="space-y-4 xl:col-span-3">
            ...
            <div className="overflow-x-auto bg-gray-100 rounded-lg p-4">
              <div className="w-full overflow-x-auto">
                ...
                >
                <div className="h-8 flex items-center justify-center font-medium text-gray-600">
                </div>
                {Array.from({ length: cols }).map((_, colIndex) => (
                  <div key={`col-${colIndex}`} className="h-8 flex items-center justify-center font-medium text-gray-600">
                    {colIndex + 1}
                  </div>
                ))}
                  ...
                  return (
                    <React.Fragment key={`row-${rowIndex}`}>
                      ...
                      {rowSeats.map((seat) => {
                        ...
                        const isCombined = seat.type === 'COUPLE' && seat.span === 2;
                        const combinedSeat = isCombined ? seatsState.find(s => s.id === seat.combinedSeatId) : null;
                          ...
                          >
                            <div
                              ...
                              className={`flex items-center justify-center font-medium
                                ...
                                ${seat.selected ? 'ring-2 ring-blue-500 ring-offset-1 z-10' : ''}
                              `}
                              style={{
                                height: '40px',
                                backgroundColor: seat.locked 
                                  ...
                                  : seat.status 
                                    ? (seatTypeColor[seat.type] || seatTypeColor['NORMAL'])
                                    : seatTypeColor['DELETED'],
                                border: `1px solid ${
                                  seat.locked 
                                    ? darkenColor(seatTypeColor.LOCKED, 10)
                                    : seat.status 
                                      ? darkenColor(seatTypeColor[seat.type] || seatTypeColor['NORMAL'], 10)
                                      : '#d1d5db'
                                }`,
                                ...
                                cursor: seat.locked ? 'not-allowed' : 'pointer'
                              }}
                            >
                              {isCombined ? 
                                `${seat.label.split('-')[0].slice(0,1)}${seat.label.split('-')[0].slice(1)}-${seat.label.split('-')[1]?.slice(1) || ''}` : 
                                seat.label.slice(1)
                              }
                            </div>
                          </Dropdown>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2 pt-2 border-t border-gray-200">
                  <Button 
                    ...
                    className="w-full"
                  >
                    {isCombining ? 'Đang chọn ghế để ghép...' : 'Ghép ghế đôi'}
                  </Button>
                  <Button 
                    icon={<DeleteOutlined />}
